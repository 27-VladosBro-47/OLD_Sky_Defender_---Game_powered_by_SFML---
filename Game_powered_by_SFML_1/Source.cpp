...
class ElementsOfMenu
{
	...
	string texture_file;
	float x, y, width, height, d_x = 1, d_y = 1, speed = 1;
	float xx, yy;
	int type;
...
public:
	...
	int CurrentFrame = 0;
	ElementsOfMenu(int Type, Image &element_image, int W, int H, float s)
	{
	...
	void ElementMove(float time)
	{
		...
		case 0:
		{
			...
			if (dir == 0)
			{
				...
				{
					element_sprite.setTextureRect(IntRect(0, 144, width, height));
				}
				if (CurrentFrame > 100 && CurrentFrame <= 200)
				{
					element_sprite.setTextureRect(IntRect(64, 144, width, height));
				}
			}
			else
			{
				...
				if (CurrentFrame > 0 && CurrentFrame <= 100)
				{
					element_sprite.setTextureRect(IntRect(192, 144, width, height));
				}
				if (CurrentFrame > 100 && CurrentFrame <= 200)
				{
					element_sprite.setTextureRect(IntRect(128, 144, width, height));
				}
			}
			xx += d_x * speed * time;
			yy += d_y * speed * time;
			element_sprite.setPosition(xx, yy);
			break;
		}
		case 1:
		{

			element_sprite.setScale(2, 2);
			if (xx < 0 || xx > 1056.0)
			{
				d_x *= -1;
				if (dir == 1)
				...
				{
					element_sprite.setTextureRect(IntRect(112, 64, width, height));
					dir = 1;
				}
		...
		{
			element_sprite.setTextureRect(IntRect(0, 0, width, height));
			element_sprite.setPosition(300, 150);
			break;
		}
		case 3:
		{
			element_sprite.setTextureRect(IntRect(0, 352, width, height));
			element_sprite.setPosition(580, 370);
			break;
		}
		case 4:
		{
			element_sprite.setTextureRect(IntRect(0, 384, width, height));
			element_sprite.setPosition(540, 420);
			break;
		}
		}
	}
	...
	void SetColor(int a)
	{
		if (a == 0)
		{
			element_sprite.setColor(Color::White);
		}
		else
		{
			element_sprite.setColor(Color::Blue);
		}
	}
...
int main()
{
	...
	Image image_name;
	image_name.loadFromFile("images\\sky_defender1.png");
	...
	{
		if(i!=2)
			elementMenu.push_back(new ElementsOfMenu(0, image_chaika, 64, 48, 0.1));
		else
			elementMenu.push_back(new ElementsOfMenu(1, image_chaika, 112, 64, 0.1));
	}

	ElementsOfMenu Name(2, image_name, 639, 185, 0.1);
	ElementsOfMenu a1(3, image_name, 64, 16, 0.1);
	ElementsOfMenu a2(4, image_name, 144, 16, 0.1);
	...
	Text text_4("Game Over", font, 48);
	Text text_5("Press Enter to exit the menu", font, 20);
	//-----Створюємо список------
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (AAA == 1)
			{
				if (Keyboard::isKeyPressed(Keyboard::Space) && Hero.get_Life()==1)
				{
					shoot.play();
					isPressed = 1;
		...
		case 0:
		{
			sprte_menu.setColor(Color::White);
			...
			Name.ElementMove(time);
			a1.ElementMove(time);
			a2.ElementMove(time);
			window.draw(Name.ReturnSprite());
			window.draw(a1.ReturnSprite());
			window.draw(a2.ReturnSprite());

			a1.SetColor(0);
			a2.SetColor(0);
			choise = 0;
			if (IntRect(580, 370, 64, 16).contains(Mouse::getPosition(window)))
			{ 
				a1.SetColor(1);
				choise = 1; 
			}
			if (IntRect(540, 420, 144, 16).contains(Mouse::getPosition(window)))
			{ 
				a2.SetColor(1);
				choise = 2;
			}
		...
		//////////////////
			...
			if (Hero.get_Life() == 0)
			{
				...
				if (timer_game_over < 0)
				{
					...
					Hero.set_y(520);
					flag = 0;
					for (i = 1, it_1 = EnemyObjects.begin(); it_1 != EnemyObjects.end(); it_1++, i++)
					{

						(*it_1)->set_x(32 + 32 * i);
						(*it_1)->set_x(-150);
						(*it_1)->ReturnSprite().setPosition((*it_1)->get_x(), (*it_1)->get_y());
					}
		...
		case 2:
		{
			sprte_menu.setPosition(0, 0);
			///////////////////
			sprte_menu.setColor(sf::Color(60, 60, 120, 255));
			window.draw((sprte_menu));
			text_1.setPosition(530, 270);
			text_3.setPosition(650, 270);
			text_4.setPosition(450, 200);
			text_5.setPosition(390, 350);
			text_3.setString(aaa);
